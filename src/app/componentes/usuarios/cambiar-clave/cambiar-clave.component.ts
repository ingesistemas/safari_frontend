...
export class CambiarClaveComponent {
  ...
  aceptar(){
    ...
    if(this.formulario.valid){
      if(datos.password2 != datos.confirma){
        setTimeout(()=>{
        ...
        this.peticionService.peticionToken("/editar-password", datos, token).subscribe({
          next: (data) => {
            this.cargandoService.cambiarEstado(false)            
            if(data.Error == true){
  ...
  cerrarSesion(){
    this.autenticaService.actualizarEstados(false, null, '')
